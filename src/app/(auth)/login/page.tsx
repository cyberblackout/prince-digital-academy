...
function LoginForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!profile?.is_active) { setError('Your account has been deactivated'); await supabase.auth.signOut(); setLoading(false); return; }

      await supabase.from('users').update({ last_login: new Date().toISOString() }).eq('id', user.id);

      const dest = redirect || (profile.role_id <= 2 ? '/admin' : profile.role_id === 3 ? '/teacher' : '/student');
      router.push(dest);
      router.refresh();
  ...
  return (
    <div className="animate-fade-in">
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div>
          ...
          <div className="relative">
            ...
            <input type={showPw ? 'text' : 'password'} value={password} onChange={(e) => setPassword(e.target.value)} required placeholder="••••••••" className="form-input pl-10 pr-10" />
      ...
      <button onClick={handleGoogleLogin} className="w-full flex items-center justify-center gap-3 border border-gray-200 rounded-xl py-3 text-sm font-medium text-gray-600 hover:bg-gray-50 transition-colors">
...
export default function LoginPage() {
  return (
    <Suspense fallback={<div className="animate-pulse text-center py-10"><Loader2 size={24} className="animate-spin text-primary mx-auto" /></div>}>
      <LoginForm />
    </Suspense>
  );
}
