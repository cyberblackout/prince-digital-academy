...
export default function NoticesPage() {
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      const { data, count } = await supabase.from('notices').select('*', { count: 'exact' }).order('created_at', { ascending: false }).range((page-1)*pageSize, page*pageSize-1);
      setItems(data || []);
      setTotal(count || 0);
      setLoading(false);
    })();
  ...
  return (
    <div className="space-y-6">
      <div><h2 className="text-xl font-bold text-gray-800">Notices</h2><p className="text-sm text-gray-500">{total} records</p></div>
      <div className="bg-white rounded-2xl shadow-sm border overflow-hidden">
        {loading ? <div className="flex justify-center py-20"><Loader2 size={32} className="animate-spin text-primary"/></div> :
        <div className="overflow-x-auto"><table className="data-table"><thead><tr><th>Title</th><th>Target Roles</th><th>Is Pinned</th><th>Is Active</th></tr></thead><tbody>
          {items.map((item, i) => <tr key={i}><td className="text-gray-600">{truncate(String(item.title ?? '—'), 40)}</td><td className="text-gray-600">{truncate(String(item.target_roles ?? '—'), 40)}</td><td className="text-gray-600">{truncate(String(item.is_pinned ?? '—'), 40)}</td><td><span className={`badge ${item.is_active==='successful'||item.is_active===true||item.is_active==='active'?'badge-success':item.is_active==='pending'||item.is_active==='scheduled'?'badge-warning':'badge-info'}`}>{String(item.is_active)}</span></td></tr>)}
