...
export default function HRStaffPage() {
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      const { data, count } = await supabase.from('staff_details').select('*', { count: 'exact' }).order('created_at', { ascending: false }).range((page-1)*pageSize, page*pageSize-1);
      setItems(data || []);
      setTotal(count || 0);
      setLoading(false);
    })();
  ...
  return (
    <div className="space-y-6">
      <div><h2 className="text-xl font-bold text-gray-800">HR / Staff</h2><p className="text-sm text-gray-500">{total} records</p></div>
      <div className="bg-white rounded-2xl shadow-sm border overflow-hidden">
        {loading ? <div className="flex justify-center py-20"><Loader2 size={32} className="animate-spin text-primary"/></div> :
        <div className="overflow-x-auto"><table className="data-table"><thead><tr><th>Employee Id</th><th>Designation</th><th>Contract Type</th><th>Salary</th></tr></thead><tbody>
          {items.map((item, i) => <tr key={i}><td className="text-gray-600">{truncate(String(item.employee_id ?? '—'), 40)}</td><td className="text-gray-600">{truncate(String(item.designation ?? '—'), 40)}</td><td className="text-gray-600">{truncate(String(item.contract_type ?? '—'), 40)}</td><td className="font-semibold">{typeof item.salary === 'number' ? formatMoney(item.salary as number) : String(item.salary ?? '—')}</td></tr>)}
