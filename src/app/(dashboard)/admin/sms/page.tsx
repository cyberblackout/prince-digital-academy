...
export default function SMSManagementPage() {
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      const { data, count } = await supabase.from('sms_logs').select('*', { count: 'exact' }).order('created_at', { ascending: false }).range((page-1)*pageSize, page*pageSize-1);
      setItems(data || []);
      setTotal(count || 0);
      setLoading(false);
    })();
  ...
  return (
    <div className="space-y-6">
      <div><h2 className="text-xl font-bold text-gray-800">SMS Management</h2><p className="text-sm text-gray-500">{total} records</p></div>
      <div className="bg-white rounded-2xl shadow-sm border overflow-hidden">
        {loading ? <div className="flex justify-center py-20"><Loader2 size={32} className="animate-spin text-primary"/></div> :
        <div className="overflow-x-auto"><table className="data-table"><thead><tr><th>Recipient</th><th>Message</th><th>Status</th><th>Sent At</th></tr></thead><tbody>
          {items.map((item, i) => <tr key={i}><td className="text-gray-600">{truncate(String(item.recipient ?? '—'), 40)}</td><td className="text-gray-600">{truncate(String(item.message ?? '—'), 40)}</td><td><span className={`badge ${item.status==='successful'||item.status===true||item.status==='active'?'badge-success':item.status==='pending'||item.status==='scheduled'?'badge-warning':'badge-info'}`}>{String(item.status)}</span></td><td className="text-gray-400 text-xs">{item.sent_at ? formatDate(item.sent_at as string) : '—'}</td></tr>)}
