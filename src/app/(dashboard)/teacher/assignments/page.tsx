...
export default function AssignmentsPage() {
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      const { data, count } = await supabase.from('assignments').select('*', { count: 'exact' }).eq('teacher_id', user!.id).order('created_at', { ascending: false }).range((page-1)*pageSize, page*pageSize-1);
      setItems(data || []);
      setTotal(count || 0);
      setLoading(false);
    })();
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl shadow-sm border overflow-hidden">
        ...
        <div className="overflow-x-auto"><table className="data-table"><thead><tr><th>Title</th><th>Due Date</th><th>Max Score</th><th>Is Published</th></tr></thead><tbody>
          {items.map((item, i) => <tr key={i}><td className="text-gray-600">{truncate(String(item.title ?? '—'), 40)}</td><td className="text-gray-400 text-xs">{item.due_date ? formatDate(item.due_date as string) : '—'}</td><td className="text-gray-600">{truncate(String(item.max_score ?? '—'), 40)}</td><td><span className={`badge ${item.is_published==='successful'||item.is_published===true||item.is_published==='active'?'badge-success':item.is_published==='pending'||item.is_published==='scheduled'?'badge-warning':'badge-info'}`}>{String(item.is_published)}</span></td></tr>)}
