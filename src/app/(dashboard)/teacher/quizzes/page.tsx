  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl shadow-sm border overflow-hidden">
        ...
        <div className="overflow-x-auto"><table className="data-table"><thead><tr><th>Title</th><th>Passing Score</th><th>Is Active</th></tr></thead><tbody>
          {items.map((item, i) => <tr key={i}><td className="text-gray-600">{truncate(String(item.title ?? '—'), 40)}</td><td className="text-gray-600">{truncate(String(item.passing_score ?? '—'), 40)}</td><td><span className={`badge ${item.is_active==='successful'||item.is_active===true||item.is_active==='active'?'badge-success':item.is_active==='pending'||item.is_active==='scheduled'?'badge-warning':'badge-info'}`}>{String(item.is_active)}</span></td></tr>)}
