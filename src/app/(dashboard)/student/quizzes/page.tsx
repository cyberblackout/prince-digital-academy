...
export default function QuizzesPage() {
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      const { data, count } = await supabase.from('quizzes').select('*', { count: 'exact' }).eq('is_active', true).order('created_at', { ascending: false }).range((page-1)*pageSize, page*pageSize-1);
      setItems(data || []);
      setTotal(count || 0);
      setLoading(false);
    })();
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl shadow-sm border overflow-hidden">
        ...
        <div className="overflow-x-auto"><table className="data-table"><thead><tr><th>Title</th><th>Passing Score</th><th>Time Limit</th></tr></thead><tbody>
          {items.map((item, i) => <tr key={i}><td className="text-gray-600">{truncate(String(item.title ?? '—'), 40)}</td><td className="text-gray-600">{truncate(String(item.passing_score ?? '—'), 40)}</td><td className="text-gray-600">{truncate(String(item.time_limit ?? '—'), 40)}</td></tr>)}
