...
export default function MyCoursesPage() {
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      const { data, count } = await supabase.from('enrollments').select('*, courses!enrollments_course_id_fkey(title, thumbnail)', { count: 'exact' }).eq('user_id', user!.id).order('created_at', { ascending: false }).range((page-1)*pageSize, page*pageSize-1);
      setItems(data || []);
      setTotal(count || 0);
      setLoading(false);
    })();
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl shadow-sm border overflow-hidden">
        ...
        <div className="overflow-x-auto"><table className="data-table"><thead><tr><th>Status</th><th>Progress Percentage</th><th>Enrolled At</th></tr></thead><tbody>
          {items.map((item, i) => <tr key={i}><td><span className={`badge ${item.status==='successful'||item.status===true||item.status==='active'?'badge-success':item.status==='pending'||item.status==='scheduled'?'badge-warning':'badge-info'}`}>{String(item.status)}</span></td><td className="text-gray-600">{truncate(String(item.progress_percentage ?? '—'), 40)}</td><td className="text-gray-400 text-xs">{item.enrolled_at ? formatDate(item.enrolled_at as string) : '—'}</td></tr>)}
