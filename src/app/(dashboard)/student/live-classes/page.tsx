...
export default function LiveClassesPage() {
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      const { data, count } = await supabase.from('live_classes').select('*', { count: 'exact' }).eq('status', 'scheduled').order('created_at', { ascending: false }).range((page-1)*pageSize, page*pageSize-1);
      setItems(data || []);
      setTotal(count || 0);
      setLoading(false);
    })();
  ...
  return (
    <div className="space-y-6">
      <div><h2 className="text-xl font-bold text-gray-800">Live Classes</h2><p className="text-sm text-gray-500">{total} records</p></div>
      <div className="bg-white rounded-2xl shadow-sm border overflow-hidden">
        {loading ? <div className="flex justify-center py-20"><Loader2 size={32} className="animate-spin text-primary"/></div> :
        <div className="overflow-x-auto"><table className="data-table"><thead><tr><th>Title</th><th>Platform</th><th>Scheduled At</th><th>Status</th><th>Meeting Link</th></tr></thead><tbody>
          {items.map((item, i) => <tr key={i}><td className="text-gray-600">{truncate(String(item.title ?? '—'), 40)}</td><td className="text-gray-600">{truncate(String(item.platform ?? '—'), 40)}</td><td className="text-gray-400 text-xs">{item.scheduled_at ? formatDate(item.scheduled_at as string) : '—'}</td><td><span className={`badge ${item.status==='successful'||item.status===true||item.status==='active'?'badge-success':item.status==='pending'||item.status==='scheduled'?'badge-warning':'badge-info'}`}>{String(item.status)}</span></td><td className="text-gray-600">{truncate(String(item.meeting_link ?? '—'), 40)}</td></tr>)}
          {items.length===0 && <tr><td colSpan={5} className="text-center py-10 text-gray-400">No records found</td></tr>}
        </tbody></table></div>}
